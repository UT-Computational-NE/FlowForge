    ...
    with open(inputfilename, 'r') as rf:
        ...
        for i, compo in enumerate(sys._components):
            ...
            if i == 0 and sys._inBoundComp is not None:
                compo.setupFluidMesh(sys._fluidMesh, inlet=(fm.Surface(sys._inBoundComp.InletArea), None), inlet_coords=inlet_coords )
            else:
            ...
            for i in range(first_node,first_node+compo.nCell):
                ...
                for j in range(3):
                    assert((lowest_corner[j]-node._boundingBox[j])<1e-15) #asserting node bounding box be within vtk min to 1e-15
                    assert((highest_corner[j]-node._boundingBox[j+3])>-1e-15) #asserting node bounding box be within vtk max to 1e-15
